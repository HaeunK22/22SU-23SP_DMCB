...
parser.add_argument('--feat_num', '-f', type=int, default=3000)
...
def train_model(model, train_dataloader, val_dataloader, patience, n_epochs, optimizer1, optimizer2):
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        totvalid_loss = np.average(totvalid_losses)
        ...
        print_msg = (f'[{epoch:>{epoch_len}}/{n_epochs:>{epoch_len}}] \n' +
                f'TRAIN\n' + 
                f'vae : {t_vae_loss:.5f} | disentangle : {t_disentangle_loss:.5f} | surv : {t_surv_loss:.5f}\n' +
                f'Loss1 : {train_loss1:.5f}| Loss2 : {train_loss2:.5f}| train Ctd : {train_ctd:.5f}\n'
                f'VALIDATION\n' +
                f'vae : {v_vae_loss:.5f} | disentangle : {v_disentangle_loss:.5f} | surv : {v_surv_loss:.5f}\n' +
                f'Loss1 : {valid_loss1:.5f}| Loss2 : {valid_loss2:.5f}| val Ctd : {valid_ctd:.5f}')
        print(print_msg)
        ...
        early_stopping(totvalid_loss, model)
...
def test_loop(dataloader, model):
    ...
    with torch.no_grad():
        ...
        print(f"VAE loss: {vae_loss:>5f},\
            Disentangle loss: {disentangle_loss:>5f},\
            Surv loss: {surv_error:>5f}")
