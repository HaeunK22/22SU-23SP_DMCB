...
parser.add_argument('--coefficients', '-c', type=str, default='1_1_1')
...
alpha, beta, gamma = config.coefficients.split('_')
...
def train_model(model, patience, n_epochs, optimizer):
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        for batch, (X, time_batch, event_batch) in enumerate(train_dataloader, 1):
            ...
            loss = alpha*mse_loss(decoder_, X) + \
                beta*latent_loss(model.z_mean, model.z_sigma) + \
                gamma*surv_loss.forward(risk = output, times = time_batch, events = event_batch, breaks = model.output_intervals.double().to(device))
            # Backpropagation
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
        ...
        # train_c_td = np.mean(train_c_index)
        # valid_c_td = np.mean(valid_ctd)
...
def test_loop(dataloader, model):
    ...
    with torch.no_grad():
        ...
        tot_error = alpha*recon_error + beta*latent_error + gamma*surv_error
...
def visualize_loss(t_loss, v_loss):
    ...
    plt.tight_layout()
    plt.show()
    fig.savefig(str(config.epoch)+"_"+config.coefficients+'_methy_loss_plot.png', bbox_inches = 'tight')
...
def visualize_cindex(t_acc, v_acc):
    ...
    plt.tight_layout()
    plt.show()
    fig.savefig(str(config.epoch)+"_"+config.coefficients+'_methy_c-td_plot.png', bbox_inches = 'tight')
